...
The var is exported in grimoire's `~/.zshenv`; SSH non-interactive shells
may not have it — pass it inline as above when unsure.

- If the connection fails, check the service: `systemctl --user status browser-use-chrome`
  (restart it with `systemctl --user restart browser-use-chrome` — user services,
  no sudo).
- `browser-use --doctor` reporting "chrome running" but "daemon alive FAIL"
  on a headless host is the discovery mismatch above, not a broken install.
