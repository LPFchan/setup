...
4. Give each subagent a complete mutual-agreement plan, its worktree path, owned scope, constraints, and verification criteria. Tell it not to commit or push. Open the prompt with `<worktree-parent>/absolute/path/to/repo</worktree-parent>` — absolute, no trailing slash. Step 8 deletes the tree, and that line is then the only surviving record of which repo the slice belonged to.
