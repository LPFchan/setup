...
if output=$(LINUX_SETUP_SOURCE_URL="file://$source_dir" bash "$ROOT/bin/setup" install fail 2>&1); then rc=0; else rc=$?; fi
...
if output=$(HOME="$offline_home" XDG_STATE_HOME="$TMP/offline-state" LINUX_SETUP_SOURCE_URL="file://$TMP/missing" bash "$ROOT/bin/setup" --batch 2>&1); then rc=0; else rc=$?; fi
