...
The `opencodex` module independently installs npm's latest [OpenCodex](https://github.com/lidge-jun/opencodex), its login-managed proxy service, and a two-column `fzf-multicolumn` launcher. Each setup run resolves the package's `latest` tag and records the concrete version in desired state. Choose one provider and either the Claude Code or Codex harness. It installs its own provider snapshot at `~/.config/opencodex/managed-profiles.json` from the same canonical `files/claudex-profiles.json` metadata and reads credentials from the machine-local OpenCode auth store without changing the Claudex runtime or configuration. Provider publication refreshes every installed consumer. OpenCodex catalog routing is available to Codex CLI on every host; Codex Desktop history synchronization is enabled only on macOS.
