...
| `passage` | Shell access to the passage secrets MCP using the `auth` login: `passage get <folder> <item>`, `passage list <folder>`, and `passage run --env NAME=folder/item -- <command>`, which fills a child environment without printing values (`~/.local/bin/passage`) | `files/passage.sh` |
| `harnesses` | AI harness install/update, settings, and MCP enrollment for claude, codex, and hermes (where `~/.hermes/config.yaml` exists); operated lost.plus credentials come from `auth`, while external-service credentials remain in the passage MCP (`~/.local/bin/harnesses`) | `files/harnesses.sh` |
