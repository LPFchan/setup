...
| `providers` | Vault-owned provider API keys: enrollment, local cache, and mirrors into opencode `auth.json`, `.zshenv`, and Hermes `~/.hermes/config.yaml` model lists (`~/.local/bin/providers`) | `files/providers.sh` |
